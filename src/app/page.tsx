    ...
    <div className="space-y-8">
      <ContentCard variant="tui" className="p-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xl font-bold text-[var(--tui-primary)]">{label}</h2>
        </div>
      ...
      <div className="space-y-6">
        <h3 className="text-lg font-bold text-[var(--tui-primary)]">Stories</h3>
...
function PromptDisclosure({
  ...
  return (
    <ContentCard variant="tui" className="p-8">
      ...
      >
        <h2 className="text-xl font-bold text-[var(--tui-primary)]">Prompt Disclosure</h2>
        <svg
          className={`w-5 h-5 text-[var(--tui-muted)] transition-transform ${open ? "rotate-180" : ""}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      {open && (
        <div className="mt-6 space-y-6">
          <div>
            <h3 className="tui-muted mb-2 font-semibold">Source Analysis Prompt</h3>
            <div className="border border-[var(--tui-border)] bg-black p-4 tui-muted font-mono whitespace-pre-wrap max-h-48 overflow-y-auto">
              {sourcePrompt}
            </div>
          </div>
          <div>
            <h3 className="tui-muted mb-2 font-semibold">{leftLabel} — Editorial Prompt</h3>
            <div className="border border-[var(--tui-border)] bg-black p-4 tui-muted font-mono whitespace-pre-wrap max-h-48 overflow-y-auto">
              {leftPrompt}
            </div>
          </div>
          <div>
            <h3 className="tui-muted mb-2 font-semibold">{rightLabel} — Editorial Prompt</h3>
            <div className="border border-[var(--tui-border)] bg-black p-4 tui-muted font-mono whitespace-pre-wrap max-h-48 overflow-y-auto">
              {rightPrompt}
            </div>
          </div>
      ...
      {usePrismView && prismPairs.length > 0 ? (
              ...
              <div className="space-y-8">
                ...
                {prismPairs.length > 1 && (
                  <ContentCard variant="tui" className="p-6">
                    <h3 className="tui-muted mb-3 text-center font-semibold">Earlier Editions</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-1">
            <ContentCard variant="tui" className="p-6">
              <h2 className="text-lg font-semibold text-[var(--tui-primary)] mb-4">Available Editions</h2>
              <div className="space-y-2">
                {dailyEditions.map((edition) => (
                  ...
                  >
                    ...
                    <div className="tui-text-muted">{edition.topics.length} topics</div>
                  </button>
                ))}
              </div>
            </ContentCard>
          ...
          <div className="lg:col-span-3">
            {selectedEdition && (
              <div className="space-y-8">
                <ContentCard variant="tui" className="p-8">
                  <div className="border-b border-[var(--tui-border)] pb-6 mb-6">
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-2xl font-bold text-[var(--tui-primary)]">{selectedEdition.newspaperName || "Daily Edition"}</h2>
                      <span className="tui-muted">
                    ...
                    <h1 className="text-3xl font-bold text-[var(--tui-primary)] mb-4 leading-tight">{selectedEdition.frontPageHeadline}</h1>
                  </div>
                  <div className="prose prose-lg max-w-none">
                    <p className="tui-text-muted leading-relaxed whitespace-pre-wrap">{selectedEdition.frontPageArticle}</p>
                  </div>
                </ContentCard>
                ...
                {enrichedData && (
                  <ContentCard variant="tui" className="p-8">
                    <h2 className="text-xl font-bold text-[var(--tui-primary)] mb-6">Source Articles</h2>
                ...
                <ContentCard variant="tui" className="p-8">
                  <div className="flex items-center gap-2 mb-6 relative group">
                    <h2 className="text-xl font-bold text-[var(--tui-primary)]">Generation Prompt</h2>
                    <div className="relative group">
                      <svg className="w-4 h-4 text-[var(--tui-primary)] cursor-help" fill="currentColor" viewBox="0 0 20 20">
