...
export default function SourceArticleCard({ article }: { article: Article }) {
  ...
  return (
    <div className="backdrop-blur-sm bg-white/5 border border-white/10 rounded-lg p-4">
      <div className="flex justify-between items-start">
        <div>
          <Link href={`/articles/${article.id}`} className="font-semibold text-white/90 hover:text-white">
            {article.headline}
          </Link>
        ...
        {hasSources && (
          <button onClick={() => setExpanded(!expanded)} className="text-sm text-blue-300 hover:text-blue-200">
