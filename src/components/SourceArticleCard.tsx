...
export default function SourceArticleCard({
  ...
  return (
    <div
      className={
        isTui
          ? "border border-[var(--tui-border)] p-4"
          : "backdrop-blur-sm bg-white/5 border border-white/10 rounded-lg p-4"
      }
    >
      <div className="flex justify-between items-start">
        ...
        {hasSources && (
          ...
          >
            {expanded
              ? "Hide Sources"
              : `View Sources (${article.messageDids.length})`}
          </button>
        )}
      </div>
      ...
      {expanded && hasSources && (
        <div
          className={`mt-4 space-y-2 pt-4 ${isTui ? "border-t border-[var(--tui-border)]" : "border-t border-white/10"}`}
        >
          {article.messageDids.map((did, i) => {
            ...
            return (
              <a
                ...
                className={
                  isTui
                    ? "block p-3 border border-[var(--tui-border)] hover:bg-[var(--tui-hover-bg)]"
                    : "block p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-colors"
                }
              >
                <div className="flex items-start gap-2">
                  <svg
                    className={`w-4 h-4 mt-0.5 shrink-0 ${isTui ? "text-[var(--tui-primary)]" : "text-blue-300"}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <div>
                    ...
                    <p
                      className={`mt-1 ${isTui ? "tui-text-muted" : "text-xs text-white/50"}`}
                    >
                      {did.slice(0, 12)}…/{rkey.slice(0, 8)}…
                    </p>
                  </div>
                </div>
              </a>
